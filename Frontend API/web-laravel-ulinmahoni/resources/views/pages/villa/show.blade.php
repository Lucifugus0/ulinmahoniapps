...
<body class="font-inter antialiased bg-white text-gray-900 tracking-tight">
    ...
    <main>
        <div class="container mx-auto px-4 py-8">
            ...
            @php
                $villaImages = array_filter([
                    $villa['image'] ?? null,
                    $villa['image_2'] ?? null,
                    $villa['image_3'] ?? null,
                ]);
                $villaImages = array_values($villaImages);
                $primaryVillaImage = $villaImages[0] ?? null;
                $secondaryVillaImages = array_slice($villaImages, 1);
                $totalVillaImages = count($villaImages);
            @endphp
            ...
            <div x-data="{
                ...
                modalAlt: '',
                currentImageIndex: 0,
                images: @json(array_map('asset', $villaImages)),
                showImage(index) {
                ...
                <div class="relative bg-gray-100 rounded-lg overflow-hidden w-full mobile-responsive-height">
                    @if($primaryVillaImage)
                        <img src="{{ asset($primaryVillaImage) }}"
                            alt="{{ $villa['name'] ?? 'Villa Image' }}"
                            class="w-full h-full object-cover object-center cursor-pointer"
                            @click.prevent="showImage(0); modalAlt='{{ $villa['name'] ?? 'Villa Image' }}'"
                    ...
                    @if(count($secondaryVillaImages) > 0)
                        <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/50 to-transparent py-4 px-6">
                            <div class="flex space-x-3 overflow-x-auto">
                                ...
                                <div class="flex-shrink-0 w-32 h-20 rounded overflow-hidden border-2 border-white shadow-md mb-2 flex-none">
                                    <img src="{{ asset($primaryVillaImage) }}"
                                        alt="{{ $villa['name'] ?? 'Villa Image' }} - Main"
                                        @click.prevent="showImage(0); modalAlt='{{ $villa['name'] ?? 'Villa Image' }}'"
                                        class="w-full h-full object-cover cursor-pointer hover:opacity-80">
                                </div>
                                @foreach($secondaryVillaImages as $index => $image)
                                    <div class="flex-shrink-0 w-32 h-20 rounded overflow-hidden border-2 border-white shadow-md mb-2 flex-none">
                                        <img src="{{ asset($image) }}"
                                            alt="{{ $villa['name'] ?? 'Villa Image' }} - Image {{ $index + 2 }}"
                                            @click.prevent="showImage({{ $index + 1 }}); modalAlt='{{ $villa['name'] ?? 'Villa Image' }} - Image {{ $index + 2 }}'"
                                            class="w-full h-full object-cover cursor-pointer hover:opacity-80">
                                    </div>
                                @endforeach
                            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6 mb-8">
                <div class="flex flex-col lg:flex-row gap-8">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach($villa['features'] ?? [] as $feature)
                                    <span class="inline-flex items-center border border-gray-300 rounded-lg px-3 py-1.5 text-sm text-gray-700">
                                        {{ $feature }}
                                    </span>
                                @endforeach
                            </div>
                    ...
                    <div class="lg:w-1/2 lg:border-l lg:pl-8 lg:border-gray-200">
                        ...
                        @php
                            $hasOriginalPrice = !empty($villa['price']['original']) && $villa['price']['original'] > 0;
                            $hasDiscountedPrice = !empty($villa['price']['discounted']) && $villa['price']['discounted'] > 0;
                        @endphp

                        @if(!$hasOriginalPrice && !$hasDiscountedPrice)
                        ...
                        @else
                            <div class="bg-teal-50 border border-teal-100 p-4 rounded-lg">
                                ...
                                @if($hasOriginalPrice && $hasDiscountedPrice && $villa['price']['original'] != $villa['price']['discounted'])
                                    <p class="text-sm text-gray-500 mb-1">
                                        <span class="line-through">Rp{{ number_format($villa['price']['original'], 0, ',', '.') }}</span>
                                    </p>
                                @endif
                                <div class="flex items-baseline">
                                    <span class="text-2xl font-bold text-teal-700">
                                        Rp{{ number_format($villa['price']['discounted'] ?? $villa['price']['original'], 0, ',', '.') }}
                                    </span>
            ...
            <div class="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div class="lg:col-span-2">
                    ...
                    <!-- Room Facilities -->
                    @if(!empty($villa['attributes']['room_facilities']))
                    <div class="mt-8">
                    ...
                    <!-- House Rules -->
                    @if(!empty($villa['attributes']['rules']))
                    <div class="mt-8">
                    ...
                    <!-- Amenities -->
                    @if(!empty($villa['attributes']['amenities']))
                    <div class="mt-8">
                        <h3 class="text-xl font-bold text-gray-900 mb-4">{{ __('properties.sections.amenities') ?? 'Property Amenities' }}</h3>
                            ...
                            <div class="flex flex-col lg:flex-row gap-8">
                                ...
                                <div class="lg:w-1/2 flex flex-col">
                                    @php
                                        $defaultLat = -6.1754;
                                        $defaultLng = 106.8272;
                                        $lat = !empty($villa['latitude']) ? (float)$villa['latitude'] : $defaultLat;
                                        $lng = !empty($villa['longitude']) ? (float)$villa['longitude'] : $defaultLng;
                                        $bboxPadding = 0.01;
                                        $bbox = sprintf('%f,%f,%f,%f',
                                            $lng - $bboxPadding,
                                            $lat - $bboxPadding,
                                            $lng + $bboxPadding,
                                            $lat + $bboxPadding
                                    ...
                                    @endphp
                                    <div class="w-full flex-1" style="min-height: 300px;">
                                        <iframe
                                ...
                                <div class="lg:w-1/2 lg:border-l lg:pl-8 lg:border-gray-200">
                                    ...
                                    <p class="text-gray-600 mb-4">{{ $villa['location'] }}</p>

                                    @if(!empty($villa['distance']))
                    ...
                    <!-- Nearby Locations -->
                    @if(!empty($villa['nearby_locations']))
                    <div class="mt-8">
                        <h3 class="text-xl font-bold text-gray-900 mb-4">{{ __('properties.details.nearby_locations') }}</h3>
                        <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
                            @php
                                $groupedLocations = collect($villa['nearby_locations'])->groupBy('category');
                                $categoryIcons = [
                    ...
                    <div id="rooms-section" class="mt-12" x-data="{ openCategories: [] }">
                        ...
                        @php
                            $groupedRooms = collect($villa['rooms'])->groupBy('name');
                        @endphp
